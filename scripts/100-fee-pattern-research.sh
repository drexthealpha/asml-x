#!/usr/bin/env bash
# Task 7.1: verify fee-mechanism patterns BEFORE writing Solidity.
#
# THINKING: #49 skeptical (fee logic is where "it looks right" hides an exploit, because the money
# path and the accounting path are the same path), #19 critical thinking, #60 map-territory (what do
# live protocols emit, versus what a tutorial shows).
#
# EVIDENCE PATH declared before code: evidence/phase7/fee-research.md
# PASS: two primary sources with links, and one named pattern REJECTED with the specific property
# that made it wrong here. Citing a blog post and calling it research is the fake win.
set -uo pipefail
cd "$(dirname "$0")"
. ./lib.sh

OUT="$REPO/evidence/phase7/fee-research.md"
mkdir -p "$(dirname "$OUT")"

# Confirm the sources actually resolve rather than citing a URL nobody loaded.
probe() {
  printf '  %-62s %s\n' "$1" "$(curl -s -o /dev/null -m 25 -w '%{http_code}' -L "$1" 2>&1)"
}

{
echo "# Fee mechanism research, task 7.1"
echo
echo "Run $(date -u '+%Y-%m-%d %H:%M:%S UTC'). Solidity pin in this repo: 0.8.28"
echo "(contracts/foundry.toml), which matters because it is above the 0.8.24 transient-storage floor."
echo
...
{
probe "https://speedrunethereum.com/guides/erc-4626-vaults"
probe "https://ackee.xyz/blog/complete-reentrancy-hands-on-guide/"
probe "https://detectors.auditbase.com/fee-on-transfer-solidity"
probe "https://github.com/code-423n4/2022-05-cudos-findings/issues/3"
} >> "$OUT"
